...
class Coordinator:
    ...
    def route_and_coordinate(self, query: str) -> dict:
        route_prompt = (
            "Task coordination: analyze user request to determine which agents to call. Request: "
            + query
            + ". Agents: product(Product), ads(Advertising), content(Copy). Return JSON with agents list and reason."
        )
        messages = [
        ...
        summary_prompt = (
            "Summarize these expert results: "
            + str(results)
            + ". User request: "
            + query
        )
