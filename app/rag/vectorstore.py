...
# 分块: chunk_size=512, chunk_overlap=100
...
class VectorStore:
    ...
    def add_documents(self, texts, metadatas=None):
        ...
        if not self.vector_store or not self.embeddings:
            logger.warning("VectorStore not ready, cannot add documents")
            return 0
        ...
        if self.vector_store:
            try:
                return self.vector_store.max_marginal_relevance_search(
                    query, k=k, fetch_k=10, lambda_mult=0.5
                )
            except Exception as e:
                logger.warning("MMR search failed, fallback to similarity: %s", e)
