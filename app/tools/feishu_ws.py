...
def do_p2_im_message_receive_v1(data):
    ...
    try:
        ...
        logger.info("[Feishu WS] [%s] RAW EVENT (full): %s" % (track_id, raw_json[:1000]))
    except:
    ...
    try:
        ...
        logger.info("[Feishu WS] [%s] Parsed - msg_type=%s, message_id=%s, chat_id=%s" % (track_id, msg_type, message_id, chat_id))
        ...
        if msg_type == "text":
            ...
            try:
                ...
                if file_key:
                    ...
                    logger.info("[Feishu WS] [%s] File detected - Key: %s, Name: %s, Size: %d" % (track_id, file_key, file_name, file_size))
                else:
                    logger.warning("[Feishu WS] [%s] File message but no file_key found: %s" % (track_id, content_json))
                    text_content = "[文件] 无法获取文件标识"
            except Exception as e:
                text_content = "[文件] 解析失败"
                logger.error("[Feishu WS] [%s] Failed to parse file content: %s" % (track_id, str(e)))
        else:
        ...
        logger.info("[Feishu WS] [%s] Queued with file_info=%s" % (track_id, file_info is not None))
    except Exception as e:
        ...
        try:
            ...
            if file_info:
                ...
                try:
                    ...
                    logger.info("[Feishu WS] [%s] Attempting to download file_key=%s to %s" % (track_id, file_key, save_path))
                    download_result = feishu_tool.download_file(file_key, save_path)
                    monitoring_stats.record_feishu_api_call(time.time() - download_start)
                    if download_result.get("success"):
                        ...
                        logger.warning("[Feishu WS] [%s] Failed to download file: %s" % (track_id, download_result.get("error", "Unknown error")))
                except Exception as e:
                    monitoring_stats.record_feishu_api_call(time.time() - download_start, success=False)
                    logger.error("[Feishu WS] [%s] Error downloading file: %s" % (track_id, str(e)))
            else:
