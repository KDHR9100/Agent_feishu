...
### 测试与复测
- 主套件 **571 passed / 3 skipped**（+7）；评测套件 **74 passed**（+7）；mock 全量 13 画像管线验证通过。
- real 复测挂起：`token-plan` 周配额耗尽，2026-09-13 15:01（北京时间）重置后补跑 file_analyst 单画像 + 全量（目标 0.73~0.75 过门禁）。
